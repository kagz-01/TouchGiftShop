...
export function FinalCTA() {
  const socialLinks = [
    ...
    },
    {
      ...
      label: "TikTok",
      path: "M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.51a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 0010.86 4.48V13.2a8.27 8.27 0 004.77 1.52V11.3a4.83 4.83 0 01-.81-.61z",
    },
    {
  ...
  const trustBadges = [
    ...
    { emoji: "📱", text: "M-Pesa accepted" },
    { emoji: "🎁", text: "Free over KSh 5,000" },
    { emoji: "📸", text: "Photo proof" },
    { emoji: "👤", text: "Anonymous mode" },
  ];
      ...
      <div className="w-full max-w-[1200px] mx-auto px-4 md:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <Reveal delay={100}>
              <h2 className="font-display text-5xl md:text-6xl lg:text-7xl font-bold text-white leading-[0.95] mb-6">
                Ready to create
                <br />
                <span className="bg-gradient-to-r from-gold via-gold-light to-gold bg-clip-text text-transparent">
                  an unforgettable
            ...
            <Reveal delay={200}>
              <p className="text-lg text-white/65 max-w-md mb-10 leading-relaxed">
            ...
            <Reveal delay={300}>
              <div className="flex flex-col sm:flex-row gap-4">
                <Link
                  href="/shop"
                  className="group relative px-8 py-4 bg-gradient-to-r from-gold to-gold-light text-brand-deep font-bold rounded-2xl text-lg overflow-hidden transition-all duration-300 hover:shadow-[0_8px_30px_rgba(212,168,83,0.4)] hover:-translate-y-1 text-center"
                >
                  <span className="relative z-10 flex items-center justify-center gap-2">
                    Send a Gift Now
                ...
                <Link
                  href="/gift-lab"
                  className="px-8 py-4 bg-white/10 backdrop-blur-sm text-white font-semibold rounded-2xl text-lg border border-white/20 hover:bg-white/20 hover:border-white/30 transition-all duration-300 hover:-translate-y-1 text-center"
                >
            ...
            <Reveal delay={400}>
              <div className="mt-10 flex items-center gap-4">
                <span className="text-white/30 text-xs uppercase tracking-widest font-semibold">Follow us</span>
                <div className="flex items-center gap-3">
                  {socialLinks.map((s) => (
                    <a
                      ...
                      aria-label={s.label}
                      className="w-9 h-9 bg-white/8 hover:bg-gold/20 border border-white/10 hover:border-gold/40 rounded-full flex items-center justify-center text-white/40 hover:text-gold transition-all duration-300 hover:scale-110"
                    >
          ...
          <Reveal delay={200} direction="right">
            <div className="grid grid-cols-2 gap-4">
              {trustBadges.map((badge, i) => (
                <div
                  key={i}
                  className="group flex items-center gap-3 p-5 bg-white/5 hover:bg-white/10 border border-white/8 hover:border-gold/30 rounded-2xl transition-all duration-400 cursor-default"
                  style={{ transitionDelay: `${i * 60}ms` }}
                >
                  <span className="text-2xl group-hover:scale-125 transition-transform duration-300 shrink-0">
                    {badge.emoji}
                  </span>
                  <span className="text-white/75 font-medium text-sm group-hover:text-white transition-colors duration-300 leading-snug">
                    {badge.text}
                  </span>
                </div>
              ))}
