...
function ProductCard({ product }: { product: Product }) {
  return (
    <Link
      ...
      className="group flex items-center gap-4 md:gap-5 p-3 md:p-4 pr-6 card-theme border border-surface-border hover:border-brand/30 rounded-[1.5rem] hover:-translate-y-1 w-full shrink-0"
    >
      {/* Image */}
      <div className="relative w-28 h-28 md:w-36 md:h-36 rounded-[1.2rem] overflow-hidden shrink-0 card-theme shadow-inner">
        {product.image_url ? (
          <Image
          ...
          {columns.map((col, colIdx) => {
            ...
            return (
              <div key={colIdx} className="flex flex-col gap-4 card-theme shape-premium-card p-5 md:p-6">
                ...
                >
                  {/* Top fade */}
                  <div className="absolute top-0 inset-x-0 h-16 bg-gradient-to-b from-transparent to-transparent z-10 pointer-events-none" />
                  
                  <div className="absolute bottom-0 inset-x-0 h-16 bg-gradient-to-t from-transparent to-transparent z-10 pointer-events-none" />
