...
TouchGift is an online gifting platform for Kenya — flowers, hampers, personalized gifts, and gift experiences, ordered on the web or via a mobile app, paid via M-Pesa, card, or Airtel Money, and delivered same-day in Nairobi / next-day nationwide.
...
- **M-Pesa payment** as the default and most prominent option at checkout (via Pesapal's hosted checkout — see Section 5), card payment as a secondary option
...
### 3.1 Group Gifting ("Pool a Gift")
Anyone can start a group gift pool for a wedding, baby shower, office send-off, or birthday: set a title, a target amount, and a deadline. TouchGift generates a shareable link. Each contributor opens the link, enters their name and amount, and pays via their own independent checkout session (M-Pesa, card, or Airtel Money) — no single person has to collect cash from everyone and place the order themselves. A live progress bar shows the pool filling up, and the gift is automatically ordered once the target is hit (or the organizer can dispatch early at partial funding). This is the specific feature that shaped the payment provider choice in Section 5 — a shared paybill/STK flow doesn't cleanly support many independent, simultaneous payers the way a per-contributor hosted checkout session does.
...
|---|---|---|
| Product/category data entry, stock counts | **WooCommerce** (private, e.g. `admin.touchgift.co.ke`) | Staff-facing only. Public storefront actively blocked — see 5.4. Not where checkout happens. |
...
| Customer accounts | **Supabase Auth** (phone + OTP) | Not WooCommerce customer accounts — those stay unused, since checkout never happens on WooCommerce |
| Orders, group gifting pools, wishlists, reminders | **Supabase** (Postgres) | Custom logic with no WooCommerce equivalent |
| Application code, all business logic | **Next.js API routes**, version-controlled in **GitHub**, deployed on push (e.g. to Vercel) | The only system that talks to Supabase, Pesapal, WhatsApp, and WooCommerce |
| Payments | **Pesapal** (hosted checkout), called from Next.js | Never touches WooCommerce. See 5.3 for why Pesapal over calling Daraja directly. |
...
The one deliberate exception to "WooCommerce is read-only": once a payment is confirmed, TouchGift writes a stock decrement back to WooCommerce, so staff aren't looking at stale stock counts in wp-admin for sales that happened entirely outside it. This is the only write path in either direction — everything else about orders, payments, and customers stays exclusively in Supabase.

### 5.3 Payments: Pesapal, not direct Daraja STK push

The original plan (and most Kenyan checkout integrations) call Safaricom's Daraja API directly for an STK push — a prompt sent straight to one phone. That works fine for a normal single-buyer checkout, but it's an awkward fit for **group gifting pools (Section 3.1)**, which need many independent people paying at different times into the same pool. Rather than build custom logic to fire STK pushes at arbitrary contributors, TouchGift uses **Pesapal's hosted checkout**: every payment — a normal order or a single pool contribution — is its own independent Pesapal session, and Pesapal itself handles offering M-Pesa, card, or Airtel Money on its hosted page. Confirmation comes back via Pesapal's IPN (Instant Payment Notification), called server-to-server — this is the only place a payment is ever marked confirmed, never the browser redirect alone, since a closed tab or lost connection right after paying shouldn't be able to leave an order in limbo.

### 5.4 Locking down the WooCommerce frontend

Because WooCommerce is only meant for product data entry, its own storefront/checkout must never be reachable by an actual customer — if it were, someone could complete a purchase there that never touches Pesapal, Supabase, or the stock write-back, and TouchGift would have no record of it. In practice this means redirecting any anonymous visitor to the WordPress install's frontend to the real TouchGift site, while leaving `wp-admin` (staff) and the REST API (the product sync) untouched.

### 5.5 Stack summary

- **Frontend:** Next.js + Tailwind, installable PWA from day one.
- **Backend/data:** Supabase (Postgres + Auth + Storage).
- **Product data entry:** WooCommerce, private, sync-only, storefront disabled — not customer-facing.
- **Payments:** Pesapal hosted checkout (M-Pesa, card, Airtel Money).
...
Customer accounts use **Supabase Auth** (`auth.users`, phone + OTP), referenced by the `user_id` foreign keys below — not WooCommerce customer accounts, which stay unused since checkout never happens on WooCommerce. `products`/`categories` are populated by the WooCommerce sync described in Section 5.2, not entered directly. Payment tracking columns reference Pesapal (Section 5.3), not Daraja.
...
CREATE TABLE orders (
    ...
    pesapal_order_tracking_id VARCHAR(100) UNIQUE,
...
- **Business registration** and a **Pesapal merchant account** (production credentials require business verification — start this early; sandbox credentials are available immediately for development).
...
- **WooCommerce API key handling**: the key used for the stock write-back (Section 5.2) needs write access, which is a more sensitive credential than a read-only sync key — store it only as a server-side environment variable, never expose it to the browser, and rotate it if it's ever suspected of leaking.
- **WooCommerce storefront must stay blocked** (Section 5.4) — verify this after every WordPress/plugin update, since an update could theoretically reset the redirect rule and quietly reopen a checkout path TouchGift has no record of.
